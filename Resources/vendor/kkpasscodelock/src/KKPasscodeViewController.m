...
//- (void)relayout {
//    [super relayout];
//    [self.whiteView setWidth:_enterPasscodeTableView.frame.size.width];
//    [self.triangleImageView moveToBottomOfView:self.whiteView offset:-3];
//}
...
- (void)loadView
{
	...
	self.view.backgroundColor = kTextOrangeColor;
...
//    }
	...
	_enterPasscodeTableView.backgroundColor = kTextOrangeColor; //[UIColor groupTableViewBackgroundColor];
	...
	_setPasscodeTableView.backgroundColor = kTextOrangeColor; //[UIColor groupTableViewBackgroundColor];
	...
	_confirmPasscodeTableView.backgroundColor = kTextOrangeColor; //[UIColor groupTableViewBackgroundColor];
...
- (void)viewWillAppear:(BOOL)animated
{
	...
	if (_mode == KKPasscodeModeSet || _mode == KKPasscodeModeChange) {
		if (_passcodeLockOn) {
			...
			UIView *boxesView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, _enterPasscodeTableView.tableHeaderView.frame.size.width, kPasscodeBoxHeight)];
		...
		[_tableViews addObject:_enterPasscodeTableView];
		[_textFields addObject:_enterPasscodeTextField];
        NSArray *textboxes = [self boxes];
		[_boxes addObject:textboxes];
		UIView *boxesView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, _enterPasscodeTableView.tableHeaderView.frame.size.width, _enterPasscodeTableView.tableHeaderView.frame.size.height)];
		boxesView.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
		for (int i = 0; i < [[_boxes lastObject] count]; i++) {
			[boxesView addSubview:[[_boxes lastObject] objectAtIndex:i]];
		}
        [CommonLayout moveViewsToHorizontalCenterOfSuperView:textboxes];
		[_enterPasscodeTableView.tableHeaderView addSubview:boxesView];
	}
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    // When passcode screen is disappeared, allow show picker popover
...
- (void)incrementFailedAttemptsLabel
{
	...
	for (int i = 0; i < kPasscodeBoxesCount; i++) {
		[[[_boxes objectAtIndex:_currentPanel] objectAtIndex:i] setImage:[UIImage imageNamed:@"passcode_cell.png"]];
	}
	...
	_failedAttemptsLabel.frame = _failedAttemptsView.frame = CGRectMake((self.view.bounds.size.width - (size.width + 40.0f)) / 2, (IS_IPHONE ? 300 : 350), size.width + 40.0f, size.height + 10.0f);
...
- (UIView*)headerViewForTextField:(UITextField*)textField
{
	...
	UIView *headerView = [[UIView alloc] initWithFrame:CGRectMake(0.0, 0.0, width, 400.0)];
    
    self.whiteView = [[BorderView alloc] initWithFrame:CGRectMake(0, (IS_IPHONE ? 70 : 120), width, 140) borderColor:kBorderLightGrayColor borderWidths:OffsetMake(0, 0, 1, 1) superView:headerView];
    self.whiteView.backgroundColor = [UIColor whiteColor];
    self.whiteView.clipsToBounds = YES;
    self.whiteView.autoresizingMask = UIViewAutoresizingFlexibleWidth;
    
    self.triangleImageView = [CommonLayout createImageView:[self.whiteView rectAtBottom:-3 width:50 height:22] image:[UIImage imageNamed:@"white_triangle_down.png"] contentMode:UIViewContentModeScaleAspectFit superView:headerView];
    self.triangleImageView.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
    
    UIImageView *logoImageView = [[UIImageView alloc] initWithFrame:CGRectMake(width * 0.05, 10, width * 0.9, 70)];
    ...
    [self.whiteView addSubview:logoImageView];
    
	UILabel *headerLabel = [[UILabel alloc] initWithFrame:CGRectMake(0.0, 105, self.view.bounds.size.width, 30.0)];
	...
	headerLabel.font = [CommonLayout getFont:FontSizeSmall isBold:NO];
    headerLabel.adjustsFontSizeToFitWidth = YES;
    headerLabel.minimumScaleFactor = 0.3;
	headerLabel.shadowOffset = CGSizeMake(0, 1.0);
	headerLabel.shadowColor = [UIColor colorWithRed:1.0 green:1.0 blue:1.0 alpha:1.0];
	
	if ([textField isEqual:_setPasscodeTextField]) {
		_passcodeConfirmationWarningLabel = [[TopAlignedLabel alloc] initWithFrame:CGRectMake(0.0, IS_IPHONE ? 290 : 350, self.view.bounds.size.width, 60.0)];
        _passcodeConfirmationWarningLabel.numberOfLines = 2;
		_passcodeConfirmationWarningLabel.textColor = [UIColor whiteColor]; // [UIColor colorWithRed:0.3 green:0.3 blue:0.4 alpha:1.0];
		...
		_passcodeConfirmationWarningLabel.font = [CommonLayout getFont:FontSizeSmall isBold:NO isItalic:YES];
        ...
        _passcodeConfirmationWarningLabel.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
    ...
    [self.whiteView addSubview:headerLabel];
	return headerView;
}
...
- (NSArray*)boxes
{
	...
	for (int i = 0; i < kPasscodeBoxesCount; i++) {
		//UIImageView *square = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"passcode_cell.png"]];
		UIImageView *square = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"passcode_cell.png"]];
		square.frame = CGRectMake(squareX, (IS_IPHONE ? 240 : 290), width, height);
...
- (BOOL)textField:(UITextField*)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString*)string
{
    ...
    for (int i = 0; i < kPasscodeBoxesCount; i++) {
        ...
        if (i < [result length]) {
            square.image = [UIImage imageNamed:@"passcode_cell_selected.png"];
        } else {
            square.image = [UIImage imageNamed:@"passcode_cell.png"];
        }
    }
