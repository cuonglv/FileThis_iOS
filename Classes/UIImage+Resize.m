...
- (UIImage*)resizedImageToFitSize:(CGSize)size {
    BOOL needToResized = NO;
    CGSize targetSize = self.size;
    if (targetSize.width > size.width) {
        targetSize.height = targetSize.height * size.width / targetSize.width;
        targetSize.width = size.width;
        needToResized = YES;
    }
    if (targetSize.height > size.height) {
        targetSize.width = targetSize.width * size.height / targetSize.height;
    ...
    NSLog(@"-- UIImage - not resize");
    return self;
}

- (void)setToImageView:(UIImageView*)imageView {
    imageView.image = [self resizedImageToFitSize:imageView.frame.size];
}
