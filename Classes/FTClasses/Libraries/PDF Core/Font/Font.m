...
- (unichar)mappedUnicodeValue:(CharacterCode)character
{
    ...
    return (value == (unichar)NSNotFound) ? value : [self encodedUnicodeValue:character];
}
...
- (unichar)encodedUnicodeValue:(CharacterCode)character
{
    ...
    NSString *s = [NSString stringWithCString:str encoding:stringEncoding];
    if ([s length] == 0)
...
CGFloat static previousFontSize = 0;
- (CGFloat)widthOfCharacter:(unichar)character withFontSize:(CGFloat)fontSize
{
    ...
    CGFloat ret = [width floatValue] * fontSize;
    if (ret > 0)
        return ret;
    ...
    @autoreleasepool {
        UIFont *font = [UIFont fontWithName:self.baseFont size:fontSize];
        ...
        if (fontSize != previousFontSize)
        {
            ...
            FontDescriptor *descriptor = [[FontDescriptor alloc] init];
            self.fontDescriptor = descriptor;
            self.fontDescriptor.descent = font.descender * 90;
            self.fontDescriptor.ascent = font.ascender * 90;
            self.fontDescriptor.leading = font.leading * 90;
            self.fontDescriptor.capHeight = font.capHeight * 90;
            self.fontDescriptor.xHeight = font.xHeight * 90;
            
            /*self.fontDescriptor.averageWidth = font.aver;
            self.fontDescriptor.maxWidth;
            self.fontDescriptor.missingWidth;
            self.fontDescriptor.verticalStemWidth;
            self.fontDescriptor.horizontalStemHeigth;
            self.fontDescriptor.italicAngle;
            self.fontDescriptor.bounds;
            self.fontDescriptor.flags;
            self.fontDescriptor.fontName;
            self.fontDescriptor.fontFile;*/
