...
- (void)viewDidLoad
{
    ...
    rect.size.height -= 64; //Minus top bar (navigation bar and status carrier)
	...
	self.scrollView.contentSize = CGSizeMake(self.view.bounds.size.width * [self.childViewControllers count], self.view.bounds.size.height);
}
...
- (void)willAnimateRotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration {
	...
	for (viewController in self.childViewControllers) {
        ...
        if ([viewController respondsToSelector:@selector(relayout)])
        {
            [viewController performSelector:@selector(relayout)];
        }
		page++;
	}
